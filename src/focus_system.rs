...
const MAX_DIST_SQ: f32 = ((PICTURE_WIDTH / 2).pow(2) + (PICTURE_HEIGHT / 2).pow(2)) as f32;

/// Contains a point to focus on.
...
impl FocusPoint {
    ...
    pub fn new(in_field: &[u16]) -> FocusPoint {
        let best_index = (0..PICTURE_WIDTH * PICTURE_HEIGHT)
            .into_par_iter()
            .map(|idx| {
                let x = idx % PICTURE_WIDTH;
                let y = idx / PICTURE_WIDTH;

                // Exclude border stripe.
                if x < WINDOW_STEP
                    || y < WINDOW_STEP
                    || x >= PICTURE_WIDTH - WINDOW_STEP
                    || y >= PICTURE_HEIGHT - WINDOW_STEP
                {
                    return 0.0;
                }
                ...
                let (sum, sq_sum) = (-WINDOW_STEP..=WINDOW_STEP)
                    ...
                    .map(|(dx, dy)| {
                        in_field[(x + dx) as usize + ((y + dy) * PICTURE_WIDTH) as usize] as f32
                    })
        ...
        FocusPoint {
            x_pos: (best_index % PICTURE_WIDTH - PICTURE_WIDTH / 2) as f32,
            y_pos: (best_index / PICTURE_WIDTH - PICTURE_HEIGHT / 2) as f32,
        }
    }
